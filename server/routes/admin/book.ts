...
function uploadImageAndUpdate(file: formidable.File, newBook: Book, req: Request, res: Response, next) {
  fs.readFile(file.path, {encoding: 'base64'}, async function (err, data) {
    if (err) {
      next(err);
      return;
...
async function renderBookForm(req: Request, res: Response, options = {}, action = 'new'){
  ...
  res.render(`admin-book.${action}.html`, {
    types,
    categories,
    languages,
    tags,
...
router.get("/import", function(req, res){
  ...
  res.render("admin-book.import.html", options);
})
  ...
  form.parse(req, function (err, _, files) {
    ...
    fs.createReadStream(file.path)
      ...
      .on("end", async function () {
        try {
          await Promise.allSettled(rawBookList.map(async (book) => (await Book.parseBook(book)).save()));
          redirectWithOption(req, res, "/admin/book/import", {
