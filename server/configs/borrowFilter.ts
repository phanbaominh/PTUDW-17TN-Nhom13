...
export default function setupBorrowFilter(env: nunjucks.Environment) {
  ...
  env.addFilter("getProfileBookSlot", (book: Book) => {
    ...
    if (BorrowCard.isFirstStatus(card.status)){
      const outerStyle = "mt-4 sm:mt-0 sm:absolute bottom-0 insert-x-auto inline-block w-full";
      ...
      return `
      <p
        class="mt-4 sm:mt-0 sm:absolute bottom-0 w-full ${textColor} border-t-2 border-indigo-500 py-2 text-center">
